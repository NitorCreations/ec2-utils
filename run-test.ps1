...
$env:EC2_UTILS_MEASURE_COVERAGE=1

ec2 volume-from-snapshot ec2-testing-ss ec2-testing-ss F 1 -i -t foo=bar -c 'test:instance'
ec2 snapshot-from-volume ec2-testing-ss ec2-testing-ss F -w -i -t foo=bar -c 'test:instance'
sleep 30
ec2 snapshot-from-volume ec2-testing-ss ec2-testing-ss F -w -i -t foo=bar -c 'test:instance'
