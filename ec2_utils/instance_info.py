        ...
        if self.network_interfaces():
            return [
                ...
                for eni in sorted(
                    ...
                    key=lambda ni: ni["Attachment"]["DeviceIndex"],
                )
            ]
        else:
            return []
        ...
        if not self._info and is_ec2():
            ...
            if os.access(info_file_dir, os.W_OK):
                ...
                with open(info_file, "w") as outf:
                    outf.write(
                        json.dumps(
                            self._info, skipkeys=True, indent=2, default=dthandler
                        )
                    )
                    try:
...
def _get_stack_resources(stack_name, stack_region=None):
    return cloudformation(region=stack_region).describe_stack_resources(
...
def _get_instance_info(instance_id):
    ...
    if (
        ...
        and resp["Reservations"]
        and "Instances" in resp["Reservations"][0]
        and resp["Reservations"][0]["Instances"]
    ):
...
def stack_params_and_outputs_and_stack(stack_name=None, stack_region=None):
    ...
    if "CreationTime" in stack:
        stack["CreationTime"] = time.strftime(
            "%a, %d %b %Y %H:%M:%S +0000", stack["CreationTime"].timetuple()
        )
    if "LastUpdatedTime" in stack:
        stack["LastUpdatedTime"] = time.strftime(
            "%a, %d %b %Y %H:%M:%S +0000", stack["LastUpdatedTime"].timetuple()
        )
