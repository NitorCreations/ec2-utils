...
def wait_net_service(server, port, timeout=None):
    ...
    while True:
        try:
            ...
            if (
                not isinstance(err.args, tuple)
                or err[0] != errno.ETIMEDOUT
                or err[0] != errno.ECONNREFUSED
            ):
                raise
...
def prune_array(
    ...
    if ten_minutely:
        _select_kept(
            ...
            time_func,
            group_by_func,
            _start_of_ten_minutes,
            now - relativedelta(minutes=ten_minutely * 10),
            now,
        )
    if hourly:
        _select_kept(
            ...
            group_by_func,
            _start_of_hour,
            now - relativedelta(hours=hourly),
            now,
        )
    if daily:
        _select_kept(
    ...
    if weekly:
        _select_kept(
            ...
            group_by_func,
            _start_of_week,
            now - relativedelta(weeks=weekly),
            now,
        )
    if monthly:
        _select_kept(
            ...
            group_by_func,
            _start_of_month,
            now - relativedelta(months=monthly),
            now,
        )
    if yearly:
        _select_kept(
            ...
            group_by_func,
            _start_of_year,
            now - relativedelta(years=yearly),
            now,
        )
...
def _select_kept(keep, objects, time_func, group_by_func, start_func, end, now):
    ...
    for obj in objects:
        ...
        if groups[obj_group]["curr"] != groups[obj_group]["prev"] and (
            obj_time > end or end > now
        ):
            keep.add(obj)
...
def delete_selected(full_array, deleted, name_func, time_func, dry_run=False):
    ...
    for obj in full_array:
        if obj not in deleted:
            print(
                colored("Skipping " + name_func(obj), "cyan")
                + " || "
                + time.strftime("%a, %d %b %Y %H:%M:%S", time_func(obj).timetuple())
            )
        else:
            print(
                colored("Deleting " + name_func(obj), "yellow")
                + " || "
                + time.strftime("%a, %d %b %Y %H:%M:%S", time_func(obj).timetuple())
            )
            has_deleted = True
